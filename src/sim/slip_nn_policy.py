"""NN-Policy-1: frozen detect backbone + small grip policy head."""
...
class SlipPolicyHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # h (hidden) + p_slip + grip_extra
        self.fc1 = nn.Linear(hidden + 2, mlp_dim)
    ...
    def forward(self, h: torch.Tensor, p_slip: torch.Tensor, grip_extra: torch.Tensor) -> torch.Tensor:
        ...
        return torch.sigmoid(self.fc2(x).squeeze(-1)) * self.max_grip
...
class SlipDetectAndPolicy(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.policy = SlipPolicyHead(hidden=mlp_dim, mlp_dim=32, max_grip=max_grip)
